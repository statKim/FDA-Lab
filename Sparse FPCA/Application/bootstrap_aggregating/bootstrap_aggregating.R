...
for (s in 2:18) {
  ...
  print("Tuning success, Bagging start!")
  ...
  y.pred <- foreach(b=1:B, .packages=packages) %dopar% {
    ...
    #                        ranges=list(cost=2^(-10:10)))$best.model
    # fit.svm.radial <- tune(svm, y ~ ., data=train.fpc, kernel="radial",
    #                        ranges=list(cost=2^(-10:10), gamma=2^(-10:10)))$best.model
  ...
  cname <- c("logit","svm.linear","svm.radial","svm.sigmoid")
  acc <- c()
  for (j in 1:4) {
...
### Bagging to selected FPC scores
system.time({   # running time check
for (s in 2:18) {
  print(paste("s=", s, sep=""))
  # train, test split
...
save(result, file="results/result_bag_fpc.RData")
