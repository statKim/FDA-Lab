...
\begin{frame}{Introduction}{A direct approach}
	\begin{block}{The direct method}
		...
		%각 curve에서 observation이 $\forall t$에 대해 존재하지 않을 경우, 존재하는 obs으로 나머지 값들을 estimate한 후 PCA를 적용하는 방법
	\end{block}
	\begin{block}{Drawbacks}
	\begin{itemize}
		\item {
			If each curves have few observation, there are not existed unique basis coefficients.
			%각 curve의 obs이 적으면 unique한 basis coefficients 존재X ($\because n_i \ll \# $ of parameters) 
		}
		\item {
			The solution is not optimal ($\because$ perform PCA onto estimated curves)
			%Optimal한 값이 아님($\because$ estimate한 curve에 PCA를 했기 때문)
		}
	\end{itemize}
	\end{block}
\end{frame}
	...
	\begin{block}{Advantages of the mixed effects model}
		\begin{itemize}
			\item {
				...
				%curve $Y_i(t)$를 estimate할 때, $i$th curve만이 아닌 모든 observation을 사용하기 때문에 direct method를 쓰기에 각 curve가 insufficient한 경우에도 사용가능하다.
			}
			\item {
				Using MLE to estimate $\bbeta, \bGamma$
				%MLE를 사용하여 $\bbeta, \bGamma$를 estimate
			}
		\end{itemize}
	\end{block}
\end{frame}
	...
	\begin{block}{Some problems with the mixed effects method}
		\begin{itemize}
			...
			\item {
				...
				%data가 sparse한 경우, $\bGamma$의 estimate이 highly variable\\
				%$\Rightarrow$ \texttt{EM} algorithm이 converge하지 못하거나 local maximum으로 converge
			}
		\end{itemize}
	\end{block}
\end{frame}
	...
	\begin{multicols}{2}
		...
		\begin{itemize}
			\item {
				...
				%\# of knots가 증가함에 따라 curve가 wiggly해짐
			}
			\item {
				In the reduced rank model, the peak is looked clear when $Age=13$.
				%reduced rank model에서는 $Age=13$일 때 peak가 뚜렷하게 나타남 
			}
			\item {
...
\begin{frame}{The reduced rank model}{Mixed effect model}
	...
	\begin{itemize}
		...
		\item {
			...
			%then mixed effects model = reduced rank model
		}
%		\item {
%			Fitting the mixed effects model using $k$ PCs is equivalent to fitting the reduced rank model.
%			%즉, $k$개의 PC만 사용하여 mixed effects model을 fitting하면 다른 방식으로 reduced rank model을 fitting할 수 있다.
%		}
	\end{itemize}
\end{frame}
	...
	\begin{block}{Study 2}
		...
		\begin{itemize}
			...
			\item {
				...
				%The more variance is underestimated, the worse likelihood estimate becomes.
			}
		\end{itemize}
	\end{block}
\end{frame}
	...
	\begin{block}{Comparison loglikelihood with different $k$}
		\begin{itemize}
			...
			\item {
				...
				%But the increase should level off when the optimal rank is reached.
			}
			\item {
...
\begin{frame}{Comparison of the Reduced Rank Method and Classical Principal Components}
	...
	%WLOG, assume $\mathbf{B'B=I}$, minimizing the objective equation is equivalent to performing standard PCA on the spline coefficients.
	\begin{itemize}
		\item {
			If all curves are measured at the same time points, then $\mathbf{B}_i=\mathbf{B}$.
		}
