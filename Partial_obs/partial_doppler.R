################################################
### Simulation using shifted Doppler signals
...
while (num.sim < num_sim) {
  ...
  # xx <- sapply(c(0.0001, 1/3, 2/3, 1-0.0001), function(tt){doppler(gr, tt)})
  # cov.true <- cov(t(xx))
  eig.obj <- get_eigen(cov.true, work.grid)
  eig.true <- eig.obj$phi[, 1:K]
  # calculate MSE
