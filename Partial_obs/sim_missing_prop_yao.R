...
if (dist_type == "tdist") {
  file_name <- paste0("RData/", setting, "-missing-", dist_type, "_yao.RData")
} else {
  file_name <- paste0("RData/", setting, "-missing-", dist_type,
                      "-prop", out_prop*10, "_yao.RData")
}
file_name
...
# )
# load("RData/Delaigle-missing-normal-prop0_yao.RData")
# df <- rbind(
#   df,
#   data.frame(
#     f = f,
...
#        filename = paste0("figure/MISE_missing_gauss_", setting, ".eps"),
#        bg = "transparent")
...
f <- seq(0.1, 0.4, by = 0.05)   # candidate of missingness parameter
setting <- "Delaigle"
file_name <- paste0("RData/Delaigle",
...
model_name <- c("(i) Gaussian process",
                "(ii) t(3) process",
                "(iii) 10% contamination",
                "(iii) 20% contamination")
# model_name <- c("(i) Gaussian process",
#                 "(ii) t(3) process",
...
ggsave(p, width = 10, height = 10,
       filename = paste0("figure/MISE_missing_", setting, "_proposed.eps"),
       bg = "transparent")
...
p
ggsave(p, width = 10, height = 10,
       filename = paste0("figure/MISE_missing_", setting, "_yao.eps"),
       bg = "transparent")
