...
var.rob.missfd = function(x, make.pos.semidef=TRUE)
{
	rob.mu= mean.rob.missfd(x)
	n=nrow(x)
	p=ncol(x)
	...
	for(s in 1:p){
		for(t in 1:p){
			A<-vector()
			for(i in 1:n){
				A[i] =(x[i,s]-rob.mu[s])*(x[i,t]-rob.mu[t])	
			}
			rob.var[s,t]=mad(A, na.rm=T)
	...
	# # transform elliposoid to unit sphere
	# G <- matrix(0, nrow(rob.var), ncol(rob.var))
	# diag.sd <- sqrt(diag(rob.var))
	# for (i in 1:nrow(rob.var)) {
	#   for (j in 1:ncol(rob.var)) {
	#     G[i, j] <- rob.var[i, j] / (diag.sd[i] * diag.sd[j])
	#   }
	# }
	# return(G)
}
...
eigen.rob.missfd = function(R,d.t,...)
{
  # d.t is the distance between the points of the equidistant grid
  #out = rpca(R)
  out=robustSvd(R)
  if (missing(d.t)) d.t = 1/ncol(R)
  out$values = out$d*d.t
  out$vectors = out$u/sqrt(d.t)
  out
}
#############################################################
                             ...
                             gcv.print=FALSE)
{
  ...
  if (missing(phi)) {
    R = var.missfd(x)
    phi = eigen.rob.missfd(R)$vectors[,1:4]
  }
  phi = as.matrix(phi)
  nscores = ncol(phi)
  if (missing(mu)) {
  ...
  if (k.M==0) { # complete observation, no prediction needed
    ...
    eig.ROO = eigen.rob.missfd(R[obs,obs,drop=FALSE],d.t=d.t)
                       ...
                       gcv.df.factor=1,gcv.plot=FALSE,gcv.print=FALSE)
{
  ...
  if (k.M>0) {
    ...
    # obs.start=which(miss==T)[1]-1
    # obs.end=which(miss==T)[length( which(miss==T))]+1
    # if(obs.start >0){  x1.pred[miss]=x1.pred[miss]-  x1.pred[miss][1] + x1[obs.start]  }
