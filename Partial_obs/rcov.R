...
rcov <- function(dat.mat,
  ...
  for(i in 1:51){
    ...
    tmp = locScaleM(dat.mat[,i], psi = "huber", eff = 0.95, maxit = 50, tol = 1e-04,
                    na.rm = TRUE)
  ...
  std.dat = (dat.mat - matrix(M.loc, nrow=nrow(dat.mat), ncol=ncol(dat.mat), byrow = TRUE))/ matrix(M.disp, nrow=nrow(dat.mat), ncol=ncol(dat.mat), byrow = TRUE)
  rcor = matrix(nrow=ncol(std.dat), ncol=ncol(std.dat))
  rcov = matrix(nrow=ncol(std.dat), ncol=ncol(std.dat))
  
  for(i in 1:ncol(std.dat)){
    tmp.vec = (1:ncol(std.dat))[-(1:i)]
    for(j in tmp.vec){
      
      if(sum(is.na(std.dat[,i])) + sum(is.na(std.dat[,j])) > 0 ){
        naij = c(which(is.na(std.dat[,i])),which(is.na(std.dat[,j])) )  
        rcor.est = 0.25*((sd_trim((std.dat[,i]+std.dat[,j])[-naij], trim=0.2))^2 - (sd_trim((std.dat[,i]-std.dat[,j])[-naij], trim=0.2))^2)
        
      } else{
        rcor.est = 0.25*((sd_trim((std.dat[,i]+std.dat[,j]), trim=0.2))^2 - (sd_trim((std.dat[,i]-std.dat[,j]), trim=0.2))^2)
