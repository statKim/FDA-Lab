...
```{r fig2, fig.cap = "Figure 2. Sample trajectories with mean curve (Right).", fig.height = 8}
par(mfrow = c(2, 3))
for (k in 1:6) {
  if (k <= 3) {
...
for (k in 1:6) {
  ...
  res.mat[k+1, ] <- c(sqrt(rowMeans(ise.var)),
  ...
  if (k == 1) {
    ...
    res.mat[k, ] <- c(sqrt(rowMeans(ise.var)),
                      sqrt(rowMeans(ise.cov)))
  }
}


knitr::kable(res.mat, digits = 3, align = "r", caption = "Table 1. Average RMISE of variance and covariance estimation for 100 simulations.") %>% 
...
```{r fig5, fig.cap = "Figure 5. Sample trajectories with mean curve (Right).", fig.height = 4}
par(mfrow = c(1, 3))
for (k in 1:3) {
...
for (k in 1:3) {
  ...
  # res.mat2[k+1, 1:2] <- rowMeans(ise.intra)   # ISE
  # ise_sd[2, 1:2] <- apply(ise.cov, 1, sd)
  
  
  ### Extrapolation parts (S_0 \ D_0)
  ise.extra <- mapply(function(x, y) {
    cov.list <- x$cov
    ind <- get_ind_inter(y)
  ...
  if (k == 1) {
    ...
    cov.est, data.list)
    # res.mat2[k, 1:2] <- rowMeans(ise.cov)   # ISE
    # ise_sd[2, 1:2] <- apply(ise.cov, 1, sd)
...
knitr::kable(res.mat, digits = 3, align = "r", caption = "Table 2. Average RMISE of covariance estimation between diagonal and off-diagonal term.") %>% 
    kable_styling("striped", full_width = FALSE, font_size = 14) %>% 
    add_header_above(c(" " = 1, " " = 1, "$\\hat{\\sigma}_X^2$" = 2, "$\\hat{\\mathbf{C}}$" = 2))


knitr::kable(res.mat2, digits = 3, align = "r", caption = "Table 3. Average RMISE of covariance estimation between intrapolations and extrapolations.") %>% 
    kable_styling("striped", full_width = FALSE, font_size = 14) %>% 
