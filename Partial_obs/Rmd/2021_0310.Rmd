...
- $\hat{\beta} = \arg\min_{\beta} \sum_i \rho \left( \frac{y_i - x_i^T\beta}{s} \right)$
- Let $L = \sum_i \rho \left( \frac{y_i - x_i^T\beta}{s} \right).$
- Normal equation : 
  $$
  \frac{\partial L}{\partial \beta} = \sum_i x_i \psi \left( \frac{y_i - x_i^T\beta}{s} \right) = 0, \text{ where } \psi(x) = \rho'(x)
  $$
- Set $w_i = \psi \left( \frac{y_i - x_i^T\beta}{s} \right) / \left( y_i - x_i^T\beta \right).$
- Then a normal equation be as 
  $$
  \sum_i x_iw_i(y_i - x_i^T\beta) = 0 \\
...
## IRLS for local kernel smoother
- $\hat{\beta} = \arg\min_{\beta} \sum_i \rho \left( \frac{y_i - x_i^T\beta}{s} \right)$
- Let $L = \sum_i \rho \left( \frac{y_i - x_i^T\beta}{s} \right).$
- Normal equation : 
  $$
