...
## Problems
- Huber loss로 대체하여 추정된 variance estimator($\hat{\sigma}_X^2$)는 error variance($\hat{\sigma}_0^2$)를 빼지 않은 값임. (원래는 이 값을 빼주는 것이 맞음.)
...
- bandwidth의 경우, cross-validation 코드에서의 오류 발견으로 임의로 선택한 것이라 생각하면 될 것 같습니다.

## Variance estimation
- Huber estimation을 위해 2가지(`rlm()`, `huber()` in `MASS`)를 고려. (단, 이후 과정에서는 모두 `rlm()`을 사용함)
- Yao (2005)의 경우, variance estimator가 음수가 나오기도 함. (따로 처리 안해줌)
