...
arrival <- "KLAX"   # arrival airport icao24
...
flight %>% 
    ...
    # filter(substr(estdepartureairport, 1, 1) == "L") %>% 
...
";"
    ...
    query <- gsub("\n", " ", query)   # remove "\n"
    query
    # query <- "select count(*), hour FROM state_vectors_data4 where icao24 = '406b20' AND hour >= 1570460400 AND hour <= 1570507200 AND time >= 1570462024 AND time <= 1570509868 AND NONNULLVALUE(lat) AND NONNULLVALUE(lon) group by hour order by hour desc;"
    ...
    plot(df$lon, df$lat)
    points(approx(df$time, df$lon, method = "linear", n = 51)$y, 
           approx(df$time, df$lat, method = "linear", n = 51)$y,
           col = 2)
