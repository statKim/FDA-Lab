...
### All flights of Incheon to LA during 2019-03-01 ~ 2019-06-30
flight_to_LA <- flight %>% 
    filter(estarrivalairport == "KLAX") %>% 
    mutate(firstseen = unixtime2date(firstseen),
           lastseen = unixtime2date(lastseen),
           ...
           duration = lastseen - firstseen)
flight_to_LA   # 169 flights


df <- flight_state %>% 
    filter(icao24 == "71be26" & callsign == "AAR202") %>% 
    mutate(time = unixtime2date(time),
           hour = unixtime2date(hour),
           minute = substr(time, 1, 16),
           day = substr(time, 1, 10)) %>% 
    filter(time >= flight_to_LA$firstseen[2] & time <= flight_to_LA$lastseen[2])
...
par(mfrow = c(4, 4))
for (i in 1:nrow(flight_to_LA)) {
    df <- flight_state %>% 
        filter(icao24 == flight_to_LA$icao24[i] & callsign == flight_to_LA$callsign[i]) %>% 
        mutate(time = unixtime2date(time),
               hour = unixtime2date(hour),
               minute = substr(time, 1, 16),
               day = substr(time, 1, 10)) %>% 
        filter(time >= flight_to_LA$firstseen[i] & time <= flight_to_LA$lastseen[i])
