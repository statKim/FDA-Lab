...
- interprete하기 쉬운 some rotated functions을 찾을 수 있을까?

  => $VARIMAX$ rotation

- Let $\bold{B}$ : $K \times n$ matrix representing the first $K$ PC functions $\xi_1,...,\xi_K$,

  지금은 $\bold{B}_{m \times n}$가 $\xi_m(t_1),...,\xi_m(t_n)$의 row를 가지고, 구간 $\bold{\tau}$에서 $n$개의 equally spaced argument values를 가진다고 가정
...
- $VARIMAX$ rotation은 orthonormality를 유지하면서 $\psi_m$의 midium-sized values를 억제한다.
  ...
  => rotated PC scores는 더이상 uncorrelated가 아니다!!

- $VARIMAX$의 idea를 사용하는 또 다른 방법은 $\bold B$가  $n$개의 basis function $\phi$의 측면에서 $\xi_m$을 expansion하는 coefficients를 포함하고 있는 것이다.
...
- If $n \gg N$,
  ...
  - $n \times n$ matrix $\bold V$ 대신에 $\bold X$의 $SVD$, $\bold{UDW'}$를 사용

  - variance matrix는 $N\bold{V=WD^2W'}$를 만족 ($\bold V=\frac{1}{N}\bold{X'X}$)
...
- Let
  ...
  - $\tilde{\bold \xi}$ : $n$-vector of values $\xi(s_j)$ => $n \times 1$ vector​
...
- The solution of eigen equation with discretization

  - eigen values $\rho=w\lambda$

  - $\int \xi(s)^2ds=1 \Longleftrightarrow w \lVert \tilde{\bold \xi} \rVert^2$

    => $\tilde{\bold \xi}=w^{-1/2}\bold u$ if $\bold u$ is a normalized eigen vector of $\bold V$

- The eigen function $\xi$의 approximate을 구하기 위해서는 discrete values $\tilde{\bold \xi}$를 interpolation해야 한다.
  
  - 만약 the discretization values $s_j$의 간격이 좁다면(closely spaced), 하나의 interpolation method를 선택하는 것이 큰 효과를 주지 못한다. (어떤 방법을 쓰더라도 비슷비슷하다)
...
- \# of basis functions $K$ 결정을 위한 고려사항
  ...
  - original data의 discrete sampling points의 개수 $n$을 어떻게 정할지?
- $K<n$ 일 때, 어느 정도의 smoothing level을 사용할지?
  - basis function이 original function의 특성을 재현할 때(curve를 estimate), 얼마나 efficient하고 powerful한지?
...
- Matrix version (the simultaneous expansion of all $N$ curves)
  ...
  and the coefficient matrix $\bold C$ is $N \times K$

- Variance-covariance function (matrix form)
  $$
  ...
  \end{align}
  $$
  Therefore the eigen equation (8.9) can be expressed as
  $$
  ...
  $$
  N^{-1}\bold{W^{1/2}C'CW^{1/2}u} = \rho \bold u
  $$
  and compute $\bold {b=W^{-1/2}u}$ for each eigen vector.

- 2가지 경우, 특별한 주의를 기울여야한다.

  - the basis가 orthonormal ($\bold W=I$)인 경우, functional PCA problem은 결국 coefficient array $\bold C$의 standard multivariate PCA로 감소하게 되고, order $K$ symmetric array $N^{-1}\bold{C'C}$의 eigen analysis만 수행하면 된다. 
  ...
  - Observed function의 개수가 너무 많지 않을 경우, observed function $x_i$를 their own basis expansion으로 보는 것이다. (즉, $N$개의 basis functions가 존재하고 그것이 observed functions이다.)
    ...
    즉, $\bold{C=I}$가 되고, problem은 $w_{ij}=\int x_i x_j$의 entry(성분)를 가지는 symmetric matrix $N^{-1}\bold W$의 eigen analysis 중 하나가 된다.
    
    - 위의 entry $w_{ij}$는 quadrature technique(구적법)으로 계산 가능

- Basis function approach로 구할 수 있는 eigen function의 최대 개수는 원칙적으로 basis의 dimension인 $K$이다.

- 그러나 basis expansion이 observed functions의 근사치를 포함하고 있다면, K eigen functions의 small proportion을 계산하기 위해 K항까지 확장하는 것은 적절하지 않다.
...
- 다양한 objectivs에 적용하도록 조작할 수 있는 근사치의 3가지 측면
  - $n$ : the number of discrete argument values $s_j$
  - $s_j$ : the argument values, called $quadrature \ points$
  - $w_j$ : the weights, called $quadrature \ weights$, attached to each function
    value in the sum.
...
- Integrand(피적분함수)의 적절한 additional conditions 아래, fixed $n$에서 더 높은 accuracy를 구하는 quadrature weights와 points를 정의
...
- The approximately matrix eigen analysis problem is
  $$
  \bold{VW}\tilde \xi=\rho \tilde \xi
  $$
  where the orthonormality condition is
  $$
  \tilde \xi_m'\bold{W}\tilde \xi_m=1 \text{ and } 
  \tilde \xi_{m_1}'\bold{W}\tilde \xi_{m_2}=0, \ m_1 \ne m_2
  $$
...
- 대부분의 quadrature schemes는 positive weights를 쓰기 때문에 8.4.2의 계산과 유사하게 approximate eigen equation의 standard form을 얻을 수 있다.
  ...
  $$
  where $\bold u=\bold W^{1/2} \tilde{\xi}$ and $\bold{u'u}=1$
